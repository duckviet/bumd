...
export class DatabaseDeployStore implements DeployStore {
  ...
  public async createQueuedVersion(input: {
    ...
    const orgId = await this.resolveOrgId(input.orgSlug);
    const docId = await this.resolveDocId(orgId, input.docSlug);
    let branchId = await this.findBranchId(docId, input.branchSlug);
    if (branchId === null) {
    ...
    const seqResult = await this.pool.query<{ readonly max_seq: number }>(
  ...
  public async markVersionFailed(versionId: string): Promise<VersionRecord> {
    ...
    await this.pool.query('UPDATE "ProcessingJob" SET status = \'failed\', "updatedAt" = NOW() WHERE "versionId" = $1', [versionId]);
    return this.getVersion(versionId);
  }
