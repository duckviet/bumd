...
function renderLine(line: string, index: number) {
  if (line.startsWith("### ")) {
    return (
      <h3 className="mt-7 text-lg font-semibold text-[#202020]" key={index}>
        {line.slice(4)}
      </h3>
    );
  }
  ...
  if (line.startsWith("## ")) {
    return (
      <h2 className="mt-8 border-t border-[#d9dedb] pt-6 text-2xl font-semibold text-[#202020]" key={index}>
        {line.slice(3)}
      </h2>
    );
  }
  ...
  if (line.startsWith("- ")) {
    return (
      <li className="ml-5 list-disc py-1 text-base leading-7 text-[#4d4d4d]" key={index}>
        {line.slice(2)}
      </li>
    );
  }
  ...
  return (
    <p className="text-base leading-7 text-[#4d4d4d]" key={index}>
      {line}
    </p>
  );
}
    ...
    <PortalShell>
      <PortalContainer className="max-w-4xl">
        <header className="rounded-lg border border-[#d9dedb] bg-white p-5 sm:p-6">
          ...
          <h1 className="mt-4 text-3xl font-semibold text-[#202020] sm:text-4xl">API change details</h1>
