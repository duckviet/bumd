    ...
    <PortalShell>
      <PortalContainer className="max-w-5xl">
        <header className="rounded-lg border border-chalk bg-white p-5 sm:p-6">
          ...
          <h1 className="mt-4 text-3xl font-semibold text-carbon sm:text-4xl">Changelog</h1>
        ...
        {changes.length === 0 ? (
          <Surface className="mt-5 p-8">
            <p className="text-sm font-semibold uppercase text-slate">No diffs yet</p>
            <h2 className="mt-3 text-2xl font-semibold text-carbon">Upload a newer version to generate a changelog.</h2>
          </Surface>
        ) : (
          <div className="mt-5 space-y-3">
            {changes.map((change) => (
              <Link
                className="block rounded-lg border border-chalk bg-white p-5 transition hover:border-signal-orange hover:bg-orange-50"
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <h2 className="mt-3 text-xl font-semibold text-carbon">{change.title}</h2>
                  </div>
                  <span className="text-sm font-semibold text-orange-800">View diff</span>
                </div>
              </Link>
            ))}
          </div>
