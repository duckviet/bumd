...
The current frontend is a Next.js App Router application using Lite Feature-Sliced Design. Dependencies flow from `app` to `page` to `widgets` to `features` to `entities` to `shared`; lower layers never import higher layers. The layer uses the singular name `page` to avoid Next.js legacy Pages Router discovery. App Router page files only import and default-export one route-level public API. `page` slices own access checks, redirects, data loading, and screen composition; route handlers remain in `app` for transport parsing and mutations. Widgets compose visible regions. Features own user interactions. Entities own normalized domain models. `shared/ui` and generic shared utilities remain domain-neutral. `shared/api` is the intentional boundary-client exception: it may expose domain-shaped, strictly typed DTO schemas and transport functions so external responses are parsed at the boundary, but it must not own domain policy, presentation, or feature workflow state.
